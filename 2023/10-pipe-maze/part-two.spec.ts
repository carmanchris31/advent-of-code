...
import { findEnclosedTiles, findLoop, findStart, parseGrid } from "./main.js";
...
`)
  ...
  const loop = findLoop(grid, start);
  const enclosed = findEnclosedTiles(grid, loop);
  expect(enclosed.map((t) => t.position)).toEqual(
    expect.arrayContaining(expected.map((t) => t.position))
  );
  expect(enclosed.length).toEqual(4);
});
...
`)
  ...
  expect(enclosed.length).toEqual(8);
});
...
`)
  ...
  expect(enclosed.length).toEqual(10);
});
...
test("real", () => {
  ...
  expect(enclosed.length).toEqual(455);
});
