...
import { findLoop, findStart, parseGrid } from "./main.js";
...
`;
  ...
  expect(Math.ceil(loop.length / 2)).toEqual(4);
});

test("example - complex", () => {
  const input = `
  ..F7.
.FJ|.
SJ.L7
...
`;
  ...
  expect(Math.ceil(loop.length / 2)).toEqual(8);
});
...
`;
  ...
  expect(Math.ceil(loop.length / 2)).toEqual(6956);
});
